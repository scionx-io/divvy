/**
 * Script to register an operator on the PaymentSplitter contract on Nile testnet
 * 
 * Usage: node scripts/register-operator-nile.js <operator_address> [fee_destination]
 * Example: node scripts/register-operator-nile.js TCPh7Qd7DwHvphmfJGCQQgCGRP7aY4drEV
 */
...
async function registerOperatorNile(operatorAddress, feeDestination = null) {
  console.log('🚀 Starting operator registration on Nile testnet...\n');

  // Validate operator address
  if (!operatorAddress) {
    console.error('❌ Error: Operator address is required');
    console.log('Usage: node scripts/register-operator-nile.js <operator_address> [fee_destination]');
    console.log('Example: node scripts/register-operator-nile.js TCPh7Qd7DwHvphmfJGCQQgCGRP7aY4drEV');
    process.exit(1);
  }
  ...
  const tronWeb = new TronWeb({
    fullHost: 'https://nile.trongrid.io', // Nile testnet
    privateKey: process.env.PRIVATE_KEY_NILE // Use Nile private key
  });
  ...
  const contractAddress = 'TGurJmxtBsdjXxafC4t9LKX4T6jc3hhkbw';
  console.log(`📍 PaymentSplitter contract address (Nile): ${contractAddress}`);
  ...
  try {
    ...
    console.log(`\n📝 Registering operator ${operatorAddress} with fee destination ${feeDestination} on Nile...`);
    ...
    console.log('⏳ Waiting for transaction confirmation on Nile...');

    // Wait for transaction confirmation
    let attempt = 0;
    const maxAttempts = 60; // Increased for Nile network
    let txInfo = null;
      ...
      try {
        ...
        if (txInfo && txInfo.id) {
          ...
          if (txInfo.receipt && txInfo.receipt.result === 'SUCCESS') {
            ...
            console.log('\n🔍 Verifying operator registration on Nile...');
            ...
            if (isRegistered) {
              console.log(`\n🎉 Operator ${operatorAddress} has been successfully registered on Nile!`);
            } else {
              console.log(`\n❌ Operator registration verification failed!`);
            }
            
            console.log(`\n🔗 Transaction on Nile TronScan: https://nile.tronscan.org/#/transaction/${tx}`);
    ...
    if (!txInfo || !txInfo.id) {
      ...
      console.log(`   - Transaction ID: ${tx}`);
      console.log(`   - You can check the status manually on Nile TronScan`);
      process.exit(1);
    }
...
if (require.main === module) {
  registerOperatorNile(operatorAddress, feeDestination)
    .then(() => {
...
module.exports = { registerOperatorNile };